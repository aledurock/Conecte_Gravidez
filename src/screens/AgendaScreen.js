...
// Componente de Status (Preservado)
...
// Componente Principal (Lógica preservada)
const AgendaScreen = () => {
  const { appointments, addReminder, editReminder, deleteReminder } = useAuth();
  ...
  const handleSaveReminder = () => {
    ...
    if (selectedAppointment) {
      editReminder(selectedAppointment.id, newReminder);
    } else {
    ...
    setTimeout(() => {
      Alert.alert(
        ...
        [
          ...
          { text: "Excluir", style: "destructive", onPress: () => { deleteReminder(selectedAppointment.id); } }
        ]
      );
    }, 150);
  };
    ...
    <SafeAreaView style={styles.safeArea}>
        ...
        <View style={styles.addModalOverlay}>
          <View style={styles.addModalContainer}>
            ...
            <View style={styles.addModalContent}>
              <TextInput style={styles.addModalInputTitle} placeholder="Adicionar Título" placeholderTextColor={COLORS.gray}
                value={newReminder.title} onChangeText={text => setNewReminder(prev => ({ ...prev, title: text }))} />
              ...
              <View style={styles.addModalRow}>
                ...
                <TextInput
                  style={[styles.addModalText, { flex: 1, marginLeft: 15 }]}
        ...
        <TouchableOpacity style={styles.pickerOverlay} onPress={() => setDatePickerVisible(false)}>
          <TouchableOpacity activeOpacity={1}>
            <View style={styles.miniCalendarContainer}>
              ...
              <View style={styles.miniCalendarWeekDays}>{shortDayNames.map((day, index) =>
                <Text key={index} style={styles.miniCalendarWeekDayText}>{day}</Text>)}</View>
              <View style={styles.miniCalendarDaysGrid}>
                {miniCalendarDays.map(d => (
                  <TouchableOpacity key={d.key} style={styles.miniCalendarDayContainer}
                    onPress={() => d.day && handleDaySelect(d.day)}>
                    <View style={[
                      styles.miniCalendarDay,
                      d.day === newReminder.date.getDate() && pickerDate.getMonth() === newReminder.date.getMonth() && styles.selectedDay
                    ]}>
                      <Text style={[
                        styles.miniCalendarDayText,
                        d.day === newReminder.date.getDate() && pickerDate.getMonth() === newReminder.date.getMonth() && styles.selectedDayText
                      ]}>{d.day}</Text>
                    </View>
                  </TouchableOpacity>
                ))}
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity onPress={() => {
                  setNewReminder(prev => ({ ...prev, time: item }));
        ...
        <TouchableOpacity style={styles.pickerOverlay} onPress={() => setOptionsModalVisible(false)}>
          <View style={styles.optionsContainer}>
            <TouchableOpacity style={styles.optionButton} onPress={() => {
              setOptionsModalVisible(false); toggleAddModal(selectedAppointment);
            }}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ alignItems: 'center' }}>
            {generateYearRange(currentDate.getFullYear()).map(year => (
              <TouchableOpacity key={year} onPress={() => handleYearSelect(year)}>
                <Text style={[styles.yearText, year === currentDate.getFullYear() && styles.selectedYearText]}>{year}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
        </View>
      ...
      <ScrollView>
        <View style={styles.calendar}>
          ...
          <View style={styles.daysGrid}>{days.map(d =>
            (<View key={d.key} style={styles.dayContainer}>
              <Text style={[styles.dayText, d.hasReminder && styles.reminderDayText]}>{d.day}</Text>
            </View>)
          )}</View>
        </View>
        ...
        <View style={styles.appointmentsList}>
          ...
          {appointmentsForMonth.map(app => {
            let subtitle = '';
            let indicatorColor = '';
            if (app.type === 'consulta' || app.type === 'exame') {
              subtitle = `${app.doctor || 'A confirmar'} - ${app.time}`;
              indicatorColor = COLORS.primary; // Azul
            } else {
              subtitle = app.description ? `${app.description} - ${app.time}` : app.time;
              indicatorColor = '#E53935'; // Vermelho
            }
            return (
              <View key={app.id} style={styles.appointmentCard}>
                ...
                {(app.type === 'consulta' || app.type === 'exame') && (
...
// *** ESTILOS CORRIGIDOS E ORGANIZADOS ***
...
  statusTag:{ paddingVertical :3,paddingHorizontal :8,borderRadius :20,},
...
    backgroundColor :COLORS.white ,position :'absolute',top :60,left :10,borderRadius :8,padding :10,elevation :10,
    shadowColor :'#000',shadowOffset :{ width :0,height :5 },shadowOpacity :.3 ,shadowRadius :5,maxHeight :500,
  },
...
  addModalHeader:{
    backgroundColor :COLORS.primary ,flexDirection :'row',justifyContent :'space-between',alignItems :'center',
    padding :15,borderTopLeftRadius :15,borderTopRightRadius :15
  },
  addModalHeaderText:{ color :COLORS.white ,fontSize :20,fontWeight :'bold'},
  addModalContent:{ padding :20},
  addModalInputTitle:{
    fontSize :22,paddingBottom :10,marginBottom :20,borderBottomWidth :1,borderBottomColor :COLORS.lightGray
  },
...
  optionsContainer:{
    backgroundColor :'white',borderRadius :10,padding :10,width :'60%',
    shadowColor :"#000",shadowOffset :{ width :0,height :2 },shadowOpacity :.25 ,shadowRadius :3.84,elevation :5
  },
